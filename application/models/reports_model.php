...
SUM(CASE WHEN aps.is_default =  1 THEN 1 ELSE 0 END) AS default_status_count",false);
	...
	function get_ip_detail_count($department, $unit, $area, $gender, $from_age, $to_age, $from_date, $to_date, $visit_name, $date_type='admit_date', $outcome = 0,$icd_chapter,$icd_block,$icd_code)
	{
		...
		->join('hospital', 'patient_visit.hospital_id=hospital.hospital_id', 'left')
		->join('icd_code','patient_visit.icd_10=icd_code.icd_code','left')
		->join('patient_followup','patient_visit.patient_id=patient_followup.patient_id and patient_followup.hospital_id=patient_visit.hospital_id','left')
...
function get_icd_detail_count($icdchapter,$icdblock,$icd_10,$department,$unit,$area,$gender,$from_age,$to_age,$from_date,$to_date,$visit_name,$visit_type,$outcome){
	...
	function get_order_detail($test_master,$department,$unit,$area,$test_area,$specimen_type,$test_method,$visit_type,$from_date,$to_date,$status,$type,$number,$antibiotic_id,$micro_organism_id,$sensitive,$outcome_type=0){
		...
		$this->db->select('test.test_master_id,
			test.group_id,
			test.test_date_time,
			test.test_done_by,
			test.test_approved_by,
			test.reported_date_time,
			test.test_range_id,
			test.temp_test_id,
		,test.test_id,test_order.order_id,order_date_time,age_years,age_months,age_days,
		test_sample.sample_id,test_method,test_name,department,patient.first_name, patient.last_name, patient.patient_id as h4allid,patient.gender,unit_name, area_name,
    ...
    function get_outcome_summary(){
		...
		$this->db->from('patient_visit')
		 ->join('patient','patient_visit.patient_id=patient.patient_id')
		 ->join('patient_followup','patient_followup.patient_id=patient.patient_id and patient_followup.hospital_id=patient_visit.hospital_id','left')
		 ->join('department','patient_visit.department_id=department.department_id','left')
		 ->join('unit','patient_visit.unit=unit.unit_id','left')
		 ->join('area','patient_visit.area=area.area_id','left')
